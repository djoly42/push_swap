...
/*   Updated: 2016/03/25 15:51:18 by djoly            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	before_while(int *f, int databeg, int datalast, t_pile *pile_b)
{
	f[0] = chr_forward(databeg, pile_b);
	f[1] = chr_back(databeg, pile_b);
	f[2] = chr_forward(datalast, pile_b);
	f[3] = chr_back(datalast, pile_b);
}
...
int			find_path(t_pile *pile_a, t_pile *pile_b, int *f)
{
	...
	tmp2 = pile_a->last;
	i = 4;
	before_while(f, pile_a->beg->data, pile_a->last->data, pile_b);
	while (i < TAB)
	{
		if (tmp->next)
		{
